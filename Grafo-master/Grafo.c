...
void vertices_ady(Lista G,Nodo nodo);
...
void vertices_ady(Lista G, Nodo nodoA){
	Dato idNode;
	Nodo node;
	...
	Nodo *nodo;
	Dato arista;
	/*NodoA extraccion de id del vertice */
	Vertice vertice;
	vertice = *(Vertice*)nodoA.contenido;
	idNode = (Dato)vertice.id;
	
	//idNode= capturaIdVertice();	/*captura de id de un vertice*/		
				
    node=*(Nodo*)nodos_adyacentes(G,idNode,compara);
